...
\`\`\``);
    ...
    return `* **数据图表:** 必须包含${chartCountText}Mermaid格式的数据可视化图表。**请尽量使用所有选中的图表类型，每种类型至少使用一次。**
...
- 如果需要展示对比、趋势或数值数据（原本想用柱状图/折线图），**必须改用 Markdown 表格**展示。
- 如果展示占比数据，请使用 **饼图 (Pie Chart)**。
- 如果原本需要流程交互（时序图），请改用 **流程图 (Flowchart)** 展示。

**严格规则：**
1. 每个图表必须以 \`\`\`mermaid 开始，以 \`\`\` 结束
2. **只能使用上述选中的${selectedTypes.length}种图表类型**
3. **尽量使用所有选中的图表类型，让图表类型多样化**
4. 流程图必须使用 graph LR（横向）
5. 如果需要展示数值数据，优先使用表格`;
};
...
${anchorUrl ? '10.' : '9.'} **图表要求：** 如果已勾选图表选项，**必须**在文章的正文中自然插入相应数量的 Mermaid 图表，不能遗漏。
